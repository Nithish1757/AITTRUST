...
function Carousel({ slides }) {
  ...
  const loopSlides = slides.length ? [...slides, slides[0]] : [];

  useEffect(() => {
    if (!slides.length) return undefined;

    const autoplay = window.setInterval(() => {
      setCurrent((activeSlide) => activeSlide + 1);
    }, 1500);
  ...
  const handleTransitionEnd = () => {
    ...
    // The final card is a copy of the first, so resetting here is invisible.
  ...
  return (
    ...
    >
      <ul
        className={`absolute flex mx-[-4vmin] ease-in-out ${
          isTransitioning ? "transition-transform duration-500" : ""
        }`}
        style={{
          transform: `translateX(-${current * (100 / loopSlides.length)}%)`,
        }}
